...
CATEGORIES={'All':None,'Private Lessons':'private_lessons','Group Workshops':'group_workshops','Courses':'courses'}

class CatalogueFrame(ttk.Frame):
    def __init__(self, master, app):
        super().__init__(master, padding=10); self.app=app
        self.filter_var=tk.StringVar(value='All'); self.sort_var=tk.StringVar(value='Name')
        ...
        # scroll
        canvas=tk.Canvas(self,highlightthickness=0); vsb=ttk.Scrollbar(self,orient='vertical',command=canvas.yview)
        self.list_frame=ttk.Frame(canvas); self.list_frame.bind('<Configure>',lambda e:canvas.configure(scrollregion=canvas.bbox('all')))
        canvas.create_window((0,0),window=self.list_frame,anchor='nw'); canvas.configure(yscrollcommand=vsb.set)
        ...
        self.refresh()
    def _filtered(self):
        prods=self.app.storage.load_all_products(); cat=CATEGORIES[self.filter_var.get()]
        if cat: prods=[p for p in prods if p['table']==cat]
    ...
    def add(self,prod):
        for it in self.app.cart:
            if it['table']==prod['table'] and it['id']==prod['id']: it['quantity']+=1; break
        else: self.app.cart.append({**prod,'quantity':1})
        self.app.storage.save_cart(self.app.cart); messagebox.showinfo('Added','Item added to cart')
